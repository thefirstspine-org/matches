...
/**
 * Worker for "Play the Fate" spell.
 */
@Injectable() // Injectable required here for dependency injection
export class SpellPlayTheFateGameWorker implements IGameWorker, IHasGameHookService {

  public gameHookService: GameHookService;

  readonly type: string = 'spell-play-the-fate';
  ...
  public async create(gameInstance: IGameInstance, data: {user: number}): Promise<IGameAction<IInteractionPutCardOnBoard>> {
    return {
      ...
      name: {
        ...
        fr: `Jouer Jouer le Destin`,
      },
      description: {
        en: `Play Play the Fate on a card`,
        fr: `Jouer Jouer le Destin sur une carte`,
      },
  ...
  public async execute(gameInstance: IGameInstance, gameAction: IGameAction<IInteractionPutCardOnBoard>): Promise<boolean> {
    ...
    cardDamaged.currentStats.capacities = cardDamaged.currentStats?.capacities?.filter((capacity: string) => capacity !== 'treason') || cardDamaged.currentStats?.capacities || [];
  ...
  protected getBoardCoords(gameInstance: IGameInstance, user: number): string[] {
    ...
    return gameInstance.cards
      .filter((card: IGameCard) => card.location === 'board' && ['creature', 'artifact'].includes(card.card.type) && card.coords && !card.currentStats.effects?.includes('shadow') && card.currentStats?.capacities?.includes('treason'))
