...
/**
 * Game worker for "ovil-ruin" spell.
 */
@Injectable() // Injectable required here for dependency injection
export class SpellOvilRuinGameWorker implements IGameWorker, IHasGameHookService {

  public gameHookService: GameHookService;

  readonly type: string = 'spell-ovil-ruin';
  ...
  public async create(gameInstance: IGameInstance, data: {user: number}): Promise<IGameAction<IInteractionPutCardOnBoard>> {
    return {
      ...
      name: {
        en: `Play Ovil's Ruin`,
        fr: `Jouer une Ruine de Ovil`,
      },
      description: {
        en: `Play Ovil's Ruin on an artifact`,
        fr: `Jouer une Ruine de Ovil sur un artefact`,
      },
  ...
  public async execute(gameInstance: IGameInstance, gameAction: IGameAction<IInteractionPutCardOnBoard>): Promise<boolean> {
    ...
    this.arenaRoomsService.sendMessageForGame(
      gameInstance,
      {
        fr: `A joué une Ruine de Ovil`,
        en: `Played Ovil's Ruin`,
      },
      gameAction.user);
