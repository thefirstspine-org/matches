...
/**
 * Worker for "accelerando" spell.
 */
@Injectable()
export class SpellAccelerandoGameWorker implements IGameWorker, IHasGameHookService {
  ...
  readonly type: string = 'spell-accelerando';
  ...
  public async execute(gameInstance: IGameInstance, gameAction: IGameAction<IInteractionPutCardOnBoard>): Promise<boolean> {
    ...
    gameInstance.cards.forEach((card: IGameCard) => {
      if (card.currentStats?.capacities?.includes('requiem')) {
        ...
        card.metadata.requiemTurns = card.metadata.requiemTurns !== undefined ? card.metadata.requiemTurns - 1 : 4;
      }
    });
    ...
    this.arenaRoomsService.sendMessageForGame(
      gameInstance,
      {
        fr: `A joué Accelerando`,
        en: `Played Accelerando`,
      },
      gameAction.user);
