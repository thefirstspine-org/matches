...
/**
 * Game worker for "rage-the-fate" spell.
 */
@Injectable() // Injectable required here for dependency injection
export class SpellRageTheFateGameWorker implements IGameWorker, IHasGameHookService {

  public gameHookService: GameHookService;

  readonly type: string = 'spell-rage-the-fate';
  ...
  public async create(gameInstance: IGameInstance, data: {user: number}): Promise<IGameAction<IInteractionPutCardOnBoard>> {
    return {
      ...
      type: this.type,
      name: {
        en: `Play Rage the Fate`,
        fr: `Jouer un Enrager le Destin`,
      },
      description: {
        en: `Play Rage the Fate on a card`,
        fr: `Jouer un Enrager le Destin sur une carte`,
      },
  ...
  public async execute(gameInstance: IGameInstance, gameAction: IGameAction<IInteractionPutCardOnBoard>): Promise<boolean> {
    ...
    // Remove all the capacities and side capacities of the artifact and add growth and run
    const cardChanged: IGameCard|undefined = gameInstance.cards
      .find((c: IGameCard) => c.location === 'board' && c.coords && c.coords.x === x && c.coords.y === y);
    if (!cardChanged) {
      this.logsService.warning('Target not found', gameAction);
      return false;
    }
    cardChanged.currentStats.top.strength *= 2;
    cardChanged.currentStats.bottom.strength *= 2;
    cardChanged.currentStats.right.strength *= 2;
    cardChanged.currentStats.left.strength *= 2;
    ...
    this.arenaRoomsService.sendMessageForGame(
      gameInstance,
      {
        fr: `A joué un Enrager le Destin`,
        en: `Played Rage the Fate`,
      },
      gameAction.user);
  ...
  protected getBoardCoords(gameInstance: IGameInstance, user: number): string[] {
    ...
    return gameInstance.cards.filter((card: IGameCard) => card.location === 'board' && card.currentStats.capacities?.includes('treason') && card.coords && !card.currentStats.effects?.includes('shadow'))
