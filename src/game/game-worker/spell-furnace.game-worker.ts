...
/**
 * Worker for "furnace" spell.
 */
@Injectable()
export class SpellFurnaceGameWorker implements IGameWorker, IHasGameHookService {
  ...
  readonly type: string = 'spell-furnace';
  ...
  public async create(gameInstance: IGameInstance, data: {user: number}): Promise<IGameAction<IInteractionPutCardOnBoard>> {
    return {
      ...
      description: {
        en: `Play Furnace on a creature or an artifact`,
        fr: `Jouer Fournaise sur une créature ou un artefact`,
      },
  ...
  public async execute(gameInstance: IGameInstance, gameAction: IGameAction<IInteractionPutCardOnBoard>): Promise<boolean> {
    ...
    const requiemCardsCount: number = gameInstance.cards.filter((card: IGameCard) =>
      card.location === 'board' && card.currentStats?.capacities?.includes('burdenEarth')
    ).length;
